...
expiration_time = datetime(2023, 9, 8, 10) # Expiration date and time of the option (y, m, d, h)
...
print(f"Predicted {coin} call option price at strike $ {K} with expiration time {expiration_time} is $ {predicted_price_at_K:.2f}")
print(f"Probability of {coin} trading on or above {K} on {expiration_time} is {100 * probability_ITM:.2f} %")
print(f"Value of shares : YES - {100 * price_yes:.2f} ¢\n"
      f"                   NO - {100 * price_no:.2f} ¢")

# Plot the predicted option prices against the strike prices
plt.title(f"Predicted {coin} Call Option Prices VS Strike Price ({expiration_time})")
plt.plot(strikes, prices, 'ro', ms=3)
min_s = min(strikes)
max_s = max(strikes)
range_s = max_s - min_s
values = np.linspace(min_s - range_s * 0.2, max_s + range_s * 0.2, 100000)
plt.plot(K, predicted_price_at_K, 'go', ms=5)
